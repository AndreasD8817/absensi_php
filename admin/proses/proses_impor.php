...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_FILES['file_absensi'])) {
    ...
    if ($_FILES['file_absensi']['error'] !== UPLOAD_ERR_OK) {
        header("Location: ../impor_absensi.php?error=Terjadi kesalahan saat mengunggah file.");
        exit();
    }
    ...
    if (!$file_handle) {
        header("Location: ../impor_absensi.php?error=Gagal membuka file yang diunggah.");
        exit();
    }
    ...
    fclose($file_handle);
    header("Location: ../impor_absensi.php?sukses_masuk=$sukses_masuk&sukses_pulang=$sukses_pulang&gagal_nip=$gagal_nip");
    exit();

} else {
    header("Location: ../impor_absensi.php?error=Tidak ada file yang diunggah.");
    exit();
}
?>
